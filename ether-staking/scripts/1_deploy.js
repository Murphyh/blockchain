async function main(){
    ...
    const Staking = await ethers.getContractFactory('EtherStaking',signer1);

    staking = await Staking.deploy({
        value: ethers.utils.parseEther('10')
    });

    console.log('Staking contract deoloyed to:',staking.address,"by",signer1.address)
    ...
    transaction = await staking.connect(signer2).stakeEther(30)

    data = { value: ethers.utils.parseEther('1')}
    transaction = await staking.connect(signer2).stakeEther(180)

    data = { value: ethers.utils.parseEther('1.75')}
    transaction = await staking.connect(signer2).stakeEther(180)

    data = { value: ethers.utils.parseEther('1')}
    transaction = await staking.connect(signer2).stakeEther(90)

...
// //         let stake_amount = 100;
// //         stakeID = await token.stake(stake_amount, { from: accounts[1] });
// //         // Assert on the emittedevent using truffleassert
// //         // This will capture the event and inside the event callback we can use assert on the values returned
// //         truffleAssert.eventEmitted(
// //             stakeID,
...
// //         try {
// //             await token.withdrawStake(200, 0, {from:owner});
// //         }catch(error){
...
// //         await token.withdrawStake(withdraw_amount, 0, {from:owner});
...
// //         await token.stake(1000, {from: owner});
// //         await helper.advanceTimeAndBlock(3600*20);

// //         summary = await token.hasStake(owner);
...
// //         await token.mint(accounts[3],1000);
// //         let initial_balance = await token.balanceOf(staker);
// //         // Make a stake on 200, fast forward 20 hours, claim reward, amount should be Initial balanace +4
// //         await token.stake(200, {from: staker});
// //         await helper.advanceTimeAndBlock(3600*20);

// //         let stakeSummary = await token.hasStake(staker);
// //         let stake = stakeSummary.stakes[0];
// //         // Withdraw 100 from stake at index 0 
// //         await token.withdrawStake(100, 0, { from: staker});

// //         // Balance of account holder should be updated by 104 tokens
// //         let after_balance = await token.balanceOf(staker);
...
// //             assert.fail(error);
// //         }
// //         let second_balance = await token.balanceOf(staker);
